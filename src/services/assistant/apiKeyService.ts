 ...
 * `aiGateway` — and this service is the rule about what may go into it. That split is the opposite
 * of `consentService`'s, which owns its own `app_meta` value, and the difference is the reason:
 * consent is a fact only this app cares about, while the key is the credential the gateway puts on
 * a header, so keeping the material in the module that spends it is what makes "the key exists in
 * one place" a property a test can point at rather than a habit.
 *
...
export const apiKeyService = {
  ...
  /**
   * Remove the stored key. Removing one that was never there is not a failure, so there is nothing
   * to report — the same shape every `clear` in this app has.
   */
  clear(): void {
    aiGateway.clearStoredKey()
  },
}
