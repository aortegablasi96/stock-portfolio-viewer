 ...
 *
 * Since Story #218 it sits in the sidebar's *head row*, at its right edge beside the brand — with
 * the thing it changes, rather than at the far end of the column from it (DDR-0068). Story #184
 * put it last for a measured reason, and that measurement is answered rather than ignored: the
 * head row has no width to spare for a control beside a one-line "Stock Portfolio Viewer", so the
 * name **wraps to two lines** and the head grows instead. It is never ellipsised and never
 * abbreviated; the column's two widths are untouched.
 *
 * The move also puts the control *above* the tablist, which makes the sidebar's Tab order
 * toggle → tabs → currency → panel — one stop fewer between the selected tab and its panel, since
 * the tablist is a single Tab stop with a roving `tabindex` (DDR-0029). `e2e/tab-navigation.spec.ts`
 * describes it.
 *
